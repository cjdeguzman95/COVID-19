...
def test_create_csv():
    with open(test_file, "w+") as file:
        ...
        covid_data_split_type.create_csv(writer)
    ...
    with open(test_file, "r") as file:
        ...
        assert(first_row[3] == "Deaths")
        assert(first_row[4] == "Recovered")
...
def test_update_csv():
    with open(test_file, "w+") as file:
        writer = csv.writer(file)
        covid_data_split_type.update_csv(writer, ["test1", "test2"])
...
covid_live_data = covid_data_split_type.retrieve_live_data(requests.get("https://pomber.github.io/covid19/timeseries.json").json())
